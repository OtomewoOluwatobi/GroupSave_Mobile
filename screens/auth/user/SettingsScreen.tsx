...
const styles = StyleSheet.create({
    ...
    headerTitle:  { fontSize: 26, fontWeight: '800', color: '#fff', marginBottom: 4, letterSpacing: -0.3 },
    headerSub:    { fontSize: 13, color: 'rgba(255,255,255,0.6)' },
    body:         { paddingHorizontal: 16 },
    versionRow:   { flexDirection: 'row', justifyContent: 'flex-end', marginTop: 12, marginBottom: 4 },
    versionPill: {
...
const SettingsRow: React.FC<{
    ...
    onPress?:     () => void;
    last?:        boolean;
}> = ({ icon, iconBg, iconColor, label, value, valueColor, toggle, toggleValue, onToggle, onPress, last }) => (
    <TouchableOpacity
        style={[styles.row, last && styles.rowLast]}
        onPress={onPress}
        activeOpacity={toggle || !onPress ? 1 : 0.7}
    >
        <View style={[styles.iconWrap, { backgroundColor: iconBg ?? D.accentSoft }]}>
        ...
        {toggle ? (
            ...
            <View style={styles.rowRight}>
                ...
                {onPress && <Ionicons name="chevron-forward" size={14} color={D.textMuted} />}
            </View>
        )}
    </TouchableOpacity>
);
...
const SettingsScreen: React.FC = () => {
    ...
    const tog = (key: keyof ToggleState, label: string) => {
        setS(prev => {
            const next = !prev[key];
            Toast.show({ type: 'success', text1: `${label} ${next ? 'enabled' : 'disabled'}` });
